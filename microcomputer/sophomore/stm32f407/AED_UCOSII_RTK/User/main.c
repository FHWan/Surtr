...
int main(void)
{
    ...
    uart_init(9600);             	//初始化USART1	TX-PA9  RX-PA10		电量计	电量计反面从左到右 TX RX GND (SET)
	uart2_init(115200);             //初始化USART2	TX-PA2  RX-PA3		EC20
	uart3_init(230400);				//初始化USART3	TX-PD8  RX-PD9		RTK
	uart4_init(115200);				//初始化UART4	TX-PC10 RX-PC11		人脸识别
	EC20_Init(); 					// EC20初始化
	MQTT_Init();
	KEY_Init();						//初始化按键连GND		PC1
	while(mpu_dmp_init())			//初始化MPU6050			SDA PD7	  SCL PD6
	{
		delay_ms(100);	
	}		
