...
int main(void)
{
	...
	uart3_init(115200);				//初始化USART3		人脸识别
	EC20_Init(); 					// EC20初始化
	MQTT_Init();
	KEY_Init();						//初始化按键连GND		PC3
	while(mpu_dmp_init())			//初始化MPU6050			SDA PB7	  SCL PB6
	{
		delay_ms(100);	
	}		
	MY_ADC_Init();                  //初始化ADC1通道11,光强 PC1
	BEEP_MY();						//初始化蜂鸣器		PD11
	LED_MY();						//初始化灯带			PD12蓝光 PD13红光
...
void LED_MY(void)		//LED初始化
{
    ...
    HAL_GPIO_WritePin(GPIOD,GPIO_PIN_12 | GPIO_PIN_13,GPIO_PIN_RESET);	//PD0 PD1置1 
}
