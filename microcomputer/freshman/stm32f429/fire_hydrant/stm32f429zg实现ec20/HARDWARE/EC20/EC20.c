...
void  MQTT_Init(void)	
{
    ...
    Clear_Buffer(2);
   printf("AT+QMTCONN=0,\"fire\",\"root\",\"xxxxxxxxxx\"\r\n");//去登录MQTT服务器，设备ID，账号和密码.用户根据实际需要进行更改
    delay_ms(500);
...
u8 Mqtt_Savedata(u8 *t_payload,char *Card_ID,float water_pressure,double lon,double lat,float pitch,float roll,float yaw ,float water_height)
{
	char json[]="{\"id\":\"LBWNB\",\"model_number\":\"SY49781\",\"manager\":\"XuBin\",\"card_id\":\"%s\",\"current_pressure\":%f,\"lon\":%lf,\"lat\":%lf,\"pitch\":%f,\"roll\":%f,\"yaw\":%f,\"water_height\":%f}";
		char t_json[255];
		int payload_len;
