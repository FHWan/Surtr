...
void  MQTT_Init(void)	
{
    ...
    Clear_Buffer(2);
   printf("AT+QMTCONN=0,\"867435056729619\",\"root\",\"xxxxxxxxx\"\r\n");//去登录MQTT服务器，设备ID，账号和密码.用户根据实际需要进行更改
    delay_ms(500);
...
u8 Mqtt_Savedata(u8 *t_payload,float wind,u8 *lon,u8 *lat,float gyrox,float gyroy,float gyroz,u8 temperature,u8 humidity,u8 *SN,u8 sp02,u8 hr)
{
	char json[]="{\"DeviceID\":20220513001,\"wind\":%f,\"lon\":%s,\"lat\":%s,\"gyrox\":%f,\"gyroy\":%f,\"gyroz\":%f,\"temperature\":%d,\"humi\":%d,\"SN\":%s,\"sp02\":%d,\"hr\":%d,\"firstcoltag\":1}";
		char t_json[255];
		int payload_len;
		unsigned short json_len;
		sprintf(t_json, json,wind , lon, lat,gyrox,gyroy,gyroz,temperature,humidity,SN,sp02,hr);
    ...
    strx=strstr((const char*)RxBuffer2,(const char*)"OK");//返回OK
      while(strx==NULL)
      {
       strx=strstr((const char*)RxBuffer2,(const char*)"OK");//返回OK
          ...
          {
              num=0;
              if(RxBuffer2[14]=='$'&&RxBuffer2[15]=='G'&&RxBuffer2[16]=='N'&&RxBuffer2[17]=='G'&&RxBuffer2[18]=='G'&&RxBuffer2[19]=='A')//报头
              {
                  for(t=0;t<len;t++)
                  {
