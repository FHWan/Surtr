...
void  MQTT_Init(void)	
{
    ...
    Clear_Buffer(2);
    printf("AT+QMTCONN=0,\"fire\",\"root\",\"xxxxxxx\"\r\n");//去登录MQTT服务器，设备ID，账号和密码.用户根据实际需要进行更改
    delay_ms(500);
...
//u8 Mqtt_Savedata(u8 *t_payload,float Water_Pressure,u8 *lon,u8 *lat,float gyrox,float gyroy,float gyroz,u8 *SN,float Water_Level)
//{
//    char json[]="{\"DeviceID\":20220513001,\"Water_Pressure\":%f,\"lon\":%s,\"lat\":%s,\"gyrox\":%f,\"gyroy\":%f,\"gyroz\":%f,\"SN\":%s,\"Water_Level\":%f,\"firstcoltag\":1}";
//    char t_json[255];
//    int payload_len;
//    unsigned short json_len;
//    sprintf(t_json, json,Water_Pressure , lon, lat,gyrox,gyroy,gyroz,SN,Water_Level);
