...
void  MQTT_Init(void)	
{
    ...
    Clear_Buffer();
   printf("AT+QMTCONN=0,\"orienteering\",\"root\",\"xxxxxxxxxx\"\r\n");//x为隐去的密码
    delay_ms(500);
...
{
		char json[]="{\"DeviceID\":2023,\"HR\":%s,\"SPO2\":%s,\"lon\":%s,\"lat\":%s,\"temperature\":%d,\"humidity\":%d,\"pitch\":%f,\"roll\":%f,\"yaw\":%f,\"wind\":%f,\"card_ID\":%s}";
		char t_json[255];
		int payload_len;
		unsigned short json_len;
		sprintf(t_json, json,HR,SPO2 , lon, lat,temperature, humidity, pitch, roll, yaw,wind,card_ID);
