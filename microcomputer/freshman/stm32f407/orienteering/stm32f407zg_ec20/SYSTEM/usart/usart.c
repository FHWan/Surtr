...
{ 
	...
	if((__HAL_UART_GET_FLAG(&UART2_Handler,UART_FLAG_RXNE)!=RESET))  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if((Rxcouter&0x8000)==0)//接收未完成
		{
			if(Rxcouter&0x4000)//接收到了0x0d
			{
				if(Res!=0x0a)Rxcouter=0;//接收错误,重新开始
				else Rxcouter|=0x8000;	//接收完成了 
			}
			else //还没收到0X0D
			{	
				if(Res==0x0d)Rxcouter|=0x4000;
				else
				{
					RxBuffer[Rxcouter&0X3FFF]=Res ;
					Rxcouter++;
					if(Rxcouter>(USART_REC_LEN-1))Rxcouter=0;//接收数据错误,重新开始接收	  
				}		 
			}
		}   		 
	}
