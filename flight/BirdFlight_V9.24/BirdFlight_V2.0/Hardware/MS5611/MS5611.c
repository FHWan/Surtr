...
#define MS5611Press_OSR  MS561101BA_OSR_4096  //气压计采集精度
#define MS5611Temp_OSR   MS561101BA_OSR_4096  //温度采集精度

// 气压计状态机
#define SCTemperature  	 0x01	  //开始温度转换
#define CTemperatureing  0x02  	//正在转换温度
#define SCPressure       0x03	  //开始转换气压
#define SCPressureing    0x04	  //正在转换气压值

#define MOVAVG_SIZE      20	   	//气压计滤波长度
...
static uint32_t Start_Convert_Time; 									//启动转换时的时间 us 
static int32_t  tempCache;

static float Alt_offset_Pa=0; 	//存放着0米时 对应的气压计值
...
static uint8_t temp_index=0; //队列指针
...
{  
	...
	delay_ms(1000); 					// 延时 
	MS561101BA_readPROM(); 	// 读取EEPROM中的标定值待用	
}
...
*功    能:	  MS5611BA 的运行程序，需要定期调用。来更新气压值和温度值 
*******************************************************************************/
