...
#define LSM303_CSH()  GPIO_SetBits(GPIOB, GPIO_Pin_15)
#define LSM303_CSL()  GPIO_ResetBits(GPIOB, GPIO_Pin_15)

//当前磁场的最大值和最小值
extern int16_t  Mag_maxx,Mag_maxy,Mag_maxz,
		 Mag_minx,Mag_miny,Mag_minz;
extern uint8_t Mag_calib; //初始化完成标志uint8_t
extern int16_t lastMx,lastMy,lastMz;
void LSM303_Initial(void);
u8 LSM303_readID(void);		//u8 == volatile uint8_t == volatile unsigned char
